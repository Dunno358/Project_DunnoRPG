...
class SkillUpgradeTest(TestCase):
    ...
    def test_invalid_character_id(self):
        self.client.login(username='testuser', password='testpass') 
        response = self.client.post(reverse('skill_upgrade', args=[100, self.normal_skill.id]))
        self.assertEqual(response.status_code, 404)
...
class SkillDowngradeTest(TestCase):
    ...
    def test_skill_dowgrade_success(self):
        self.client.login(username='testuser', password='testpass')
        
        #Magical skill
        url = reverse('skill_downgrade', args=[self.character.id, self.magical_skill.id])
        response = self.client.post(url)
        
        self.assertEqual(response.status_code, 302) #302 for redirect
        
        updated_skill = models.Skills.objects.get(id=self.magical_skill.id)
        self.assertEqual(updated_skill.level, 1)
        self.assertEqual(updated_skill.desc, f"{self.magical_skill_desc.desc} {self.magical_skill_desc.level1}")
        ...
        #Melee skill
        url = reverse('skill_downgrade', args=[self.character.id, self.normal_skill.id])
        response = self.client.post(url)   
        ...
        updated_skill = models.Skills.objects.get(id=self.normal_skill.id)
        self.assertEqual(updated_skill.level, 1)
        self.assertEqual(updated_skill.desc, f"{self.normal_skill_desc.desc} {self.normal_skill_desc.level1}")
    ...
    def test_minimal_level(self):
        self.magical_skill.level = 1
        self.magical_skill.save()
        self.normal_skill.level = 1
        self.normal_skill.save()
        
        #Magical skill
        self.client.login(username='testuser', password='testpass')
        response = self.client.post(reverse('skill_downgrade', args=[self.character.id, self.magical_skill.id]))  
        self.assertEqual(response.status_code, 302)    
        messages = list(get_messages(response.wsgi_request))
        self.assertEqual(len(messages), 1)
        self.assertEqual(str(messages[0]), f"{self.magical_skill.skill} level cannot be lower!")
        
        #Normal skill
        response2 = self.client.post(reverse('skill_downgrade', args=[self.character.id, self.normal_skill.id]))
        self.assertEqual(response2.status_code, 302)
        messages = list(get_messages(response2.wsgi_request))
        self.assertEqual(len(messages), 2)
        self.assertEqual(str(messages[1]), f"{self.normal_skill.skill} level cannot be lower!")
        
    def test_invalid_character_id(self):
        self.client.login(username='testuser', password='testpass')
        response = self.client.post(reverse('skill_downgrade', args=[100, self.magical_skill.id])) 
        self.assertEqual(response.status_code, 404)
